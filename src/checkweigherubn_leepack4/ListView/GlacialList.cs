...
namespace GlacialComponents.Controls
{
    ...
    /// Click happened inside control.  Use ClickEventArgs to find out origination area.
    /// </summary>
    public event ClickedEventHandler SelectedSubItemChanged;
    ...
    private int m_FocusedOnColumn = (-1);
		private bool							m_bShowFocusRect = false;

    private bool m_bHotColumnTracking = true;
		private bool							m_bHotItemTracking = true;
    ...
    private Color             m_colorAlternateBackground = Color.WhiteSmoke;//Color.DarkGreen;
    ...
    private bool              m_HighLightSelectedSubItem = false;
    private Color             m_colorm_HighLightSelectedSubItem = Color.Red;
		private System.Windows.Forms.ImageList imageList1;

    private object _embeddedData;
    #region Hover
    ...
    public object ActivatedEmbeddedData
    {
      get
      {
    ...
    /// </summary>
    [
    Description("High light Selected-SubItem"),
    Category("Item"),
    Browsable(true)
    ]
    public bool HighLight_SelectedSubItem
    {
      get { return m_HighLightSelectedSubItem; }
      set { m_HighLightSelectedSubItem = value; }
    }
    ...
    [
    RefreshProperties(RefreshProperties.Repaint),
    Description("Color for SubItems that are selected."),
    Category("Header"),
    Browsable(true)
    ]
    public Color HighLightSelectedSubItemColor
    {
      get { return m_colorm_HighLightSelectedSubItem; }
      set
      {
        m_colorm_HighLightSelectedSubItem = value;
      }
    }
        ...
        if (m_FocusedItem != value)
        {
          ...
          if (this.SelectedIndexChanged != null)
            this.SelectedIndexChanged(this, new ClickEventArgs(Items.FindItemIndex(value), this.m_FocusedOnColumn));			// never a column sent with selection index change
      ...
      if (ret == true)
      {
        if (ItemChangedEvent != null)
          ItemChangedEvent(this, new ChangedEventArgs(ChangedTypes.DeactiveEmbeddedControlChanged, null, m_FocusedItem, null));        // fire the column clicked event
    ...
    public void DisplayProgressBar(int nColumn, GLItem item, GLSubItem subItem)
    {
      ActivateEmbeddedControl(nColumn, item, subItem);
    }
      ...
      try
      {
        ...
        if (this.GridLineStyle == GLGridLineStyles.gridNone)
        {
          ...
          try
          {
            control.Bounds = new Rectangle(subItem.LastCellRect.X + 1, subItem.LastCellRect.Y + 1, subItem.LastCellRect.Width - 3, subItem.LastCellRect.Height - 2); ;
          }
          catch
          {
          }
        ...
        {
          control.Location = new Point(subItem.LastCellRect.X + 1, subItem.LastCellRect.Y + 2);
          control.Width = subItem.LastCellRect.Width - 3;
          control.Height = subItem.LastCellRect.Height - 3;
          //if (icontrol)
          ...
          //  //control.Bounds = new Rectangle(subItem.LastCellRect.X + 1, subItem.LastCellRect.Y + 2, subItem.LastCellRect.Width - 3, subItem.LastCellRect.Height - 3);
          //}
          //catch
          //{
        ...
        try
        {
          //if ((control.Bounds.Location.X != controlBounds.Location.X) ||
          //    (control.Bounds.Location.Y != controlBounds.Location.Y))
          //{
          //  control.Bounds.X = controlBounds.X;
          //}
          ...
          //
          if (this.ActivatedEmbeddedControl != null)
          {
            if (ItemChangedEvent != null)
            {
              ItemChangedEvent(this, new ChangedEventArgs(ChangedTypes.ActiveEmbeddedControlChanged, null, m_FocusedItem, null));        // fire the column clicked event
            }
          }
        }
        catch
      ...
      catch
      {
		...
		public override bool PreProcessMessage(ref Message msg)
		{
            ...
            this.m_FocusedOnColumn = (-1);
						this.FocusedItem = null; //KeyPress
            //
						return base.PreProcessMessage(ref msg);						
					}
          bool IsNeed_Redraw = false;
          if ((keyCode == Keys.Left) && (this.HighLight_SelectedSubItem == true))
          {
            if (this.m_FocusedOnColumn >= 0)
            {
              this.m_FocusedOnColumn--;
              IsNeed_Redraw = true;
            }
          }
          else if ((keyCode == Keys.Right) && (this.HighLight_SelectedSubItem == true))
          {
            if (this.m_FocusedOnColumn < this.Columns.Count)
            {
              this.m_FocusedOnColumn++;
              IsNeed_Redraw = true;
            }
        ...
        if (this.HighLight_SelectedSubItem == false)
        {
          this.m_FocusedOnColumn = (-1);
        }

        if (this.m_FocusedOnColumn == (-1))
        {
          subItem.Selected = false;
        }

        if ((item == m_FocusedItem) && (nColumn == this.m_FocusedOnColumn) && (this.HighLight_SelectedSubItem == true))
        {
          //int mmm = 0;
          SolidBrush bbrush = new SolidBrush(this.HighLightSelectedSubItemColor);
          graphicsSubItem.FillRectangle(bbrush, rectSubItem);
          bbrush.Dispose();
        }
        ...
        {
          bool IsEmptyText = (subItem.Text == "");
          rectSubItem = DrawCheckBox(IsEmptyText, graphicsSubItem, rectSubItem, subItem.Checked);
        }
        ...
        if (subItem.ActiveHightlightColor == true)
        {
          textColor = Color.Red;
        }


        if (this.Columns[nColumn].DisplayProgressBar)
        {
          Rectangle textRectangle = DrawProgressBar(subItem.ProgresBarPercent, graphicsSubItem, rectSubItem, textColor);
          string text = ""; //String.Format("{0}%, {1}", subItem.ProgresBarPercent, subItem.Text);
          DrawCellText(graphicsSubItem, textRectangle, text, Columns[nColumn].TextAlignment, textColor, item.Selected, ItemWordWrap);
    ...
    virtual public Rectangle DrawProgressBar(int ProgresBarPercent, Graphics graphicsCell, Rectangle rectCell, Color textColor)
    {
      //int offset
      int th, ty, tw, tx;
      th = rectCell.Height - 4;
      tw = (int)((double)rectCell.Width - 0) ;
      ty = rectCell.Y + 4;
      tx = rectCell.X;
      //
      int text_th, text_ty, text_tw, text_tx;
      
      //
      Rectangle textRectangle = rectCell;
      try
      {
        //tw = (int)((valueInt / (double)rectCell.Width) * 100);
        tw = (int)((ProgresBarPercent / 100.0) * (double)tw);
        Brush brush = Brushes.Green;
        graphicsCell.FillRectangle(brush, tx, ty, tw, th);
        //
        text_tx = tx + tw;
        text_ty = ty;
        text_tw = rectCell.Width - tw;
        text_th = th;
        textRectangle = new Rectangle(text_tx, text_ty, text_tw, text_th);
      }
      catch
      {
      }
    ...
    /// <returns></returns>
    virtual public Rectangle DrawCheckBox(bool IsEmptyText, Graphics graphicsCell, Rectangle rectCell, bool bChecked )
		{
			int th, ty, tw, tx;
      ...
      //Allign center for Checkbox if EmptyText
      if (IsEmptyText == true)
      {
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
          ...
          if (Items[nItem].SubItems[nColumn].Text != "")
          {
            if (
            ...
            {
              //if (Items[nItem].SubItems[nColumn].C)if (this.Columns[nColumn].CheckBoxes)
              if (this.Columns[nColumn].CheckBoxesReadOnly == false)
              {
                // toggle the checkbox
        ...
        //
        this.m_FocusedOnColumn = nColumn;//OnMouseDown
				this.FocusedItem = Items[nItem]; //OnMouseDown
        ...
        if ((m_nLastSelectionIndex < Count) && (m_nLastSubSelectionIndex < Columns.Count))
        {
          ...
          Items[m_nLastSelectionIndex].SubItems[m_nLastSubSelectionIndex].EnableContextMenu = Columns[m_nLastSubSelectionIndex].EnableContextMenu;
        }
        if ((FullRowSelect == false) && ((nItem < Count) && (nColumn < Columns.Count)))
        {
          Items[nItem].SubItems[nColumn].Selected = true;//OnMouseDown
          Items[nItem].SubItems[nColumn].EnableContextMenu = Columns[nColumn].EnableContextMenu;
          /* just send event by SubItemSelect changed by */
          if (this.SelectedSubItemChanged != null)
            this.SelectedSubItemChanged(this, new ClickEventArgs(Items.FindItemIndex(this.FocusedItem), this.m_FocusedOnColumn));
        }
    ...
    protected override void OnMouseWheel(MouseEventArgs e)
    {
      ...
      if (vPanelScrollBar.Visible == true)
      {
        DW("OnMouseWheel true");
        //OnScroll(this, new ScrollEventArgs());
        //DI("Calling Invalidate From OnScroll");
        ...
        if (e.Delta < 0)
        {
          if (vPanelScrollBar.Value <= vPanelScrollBar.Maximum - 1)
          {
            vPanelScrollBar.Value += 1;
            this.DestroyActivatedEmbedded();
            Invalidate();
          }
          else
        ...
        {
          if (vPanelScrollBar.Value >= vPanelScrollBar.Minimum + 1)
          {
            vPanelScrollBar.Value -= 1;
            this.DestroyActivatedEmbedded();
            Invalidate();
